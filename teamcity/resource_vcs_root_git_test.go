		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceName, "auth.0.type", "userpass"),
					resource.TestCheckResourceAttr(resourceName, "auth.0.username", "admin"),
					resource.TestCheckResourceAttrSet(resourceName, "auth.0.password"),
				),
			},
		},
	})
		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckVcsRootGitExists(resourceName, &vcs),
					resource.TestCheckResourceAttr(resourceName, "auth.#", "1"),
					resource.TestCheckResourceAttr(resourceName, "auth.0.type", "ssh"),
					resource.TestCheckResourceAttr(resourceName, "auth.0.username", "admin"),
					resource.TestCheckResourceAttr(resourceName, "auth.0.ssh_type", "uploadedKey"),
					resource.TestCheckResourceAttr(resourceName, "auth.0.key_spec", "myKey"),
				),
			},
		},
	})
